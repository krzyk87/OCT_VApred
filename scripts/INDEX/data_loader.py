...
class OCTDatasetWithClinical(Dataset):
    """
    Dataset class combining OCT images with clinical features for INDEX dataset.
...
def get_transforms(image_size=496, is_training=True):
    """
    Get image transformations for training or validation for INDEX dataset.
    ...
    if is_training:
        return A.Compose([
            A.Rotate(limit=8, p=0.8, border_mode=0),
            ...
            A.GaussianBlur(blur_limit=(5, 13), p=0.3),
            A.GaussNoise(var_limit=(0.10, 8.58), mean=0, p=0.3),
            A.RandomBrightnessContrast(brightness_limit=0.1, contrast_limit=0.1, p=0.2),
            A.CoarseDropout(max_holes=8, max_height=20, max_width=20, min_holes=2, min_height=8, min_width=8, p=0.2),
